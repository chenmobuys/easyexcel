 ...
 */
class XlsReader extends Reader
{
    // Record constant
    ...
    protected function loadFromFile(string $filename): ReaderInterface
    {
        ...
        $this->readSheetsAndStyles();

        $this->readSheetInformation();
     ...
     * @return void
     */
    protected function readSheetsAndStyles(): void
    {
        $currentXfIndex = 0;
        $position = 0;
            ...
            switch ($code) {
                ...
                case self::RECORD_SST:
                    ...
                    for ($i = $uniqueStringsCount; $i--;) {
                        ...
                        $this->addSharedString($sharedString);
                    }
                    break;
                case self::RECORD_SHEET:
                    ...
                    $this->getSheetByName($name, true);

                    $this->sheetsData[] = compact('name', 'type', 'offset', 'state');
     ...
     * @return void
     */
    protected function readSheetInformation(): void
    {
        foreach ($this->sheetsData as $index => $sheet) {
            $totalRows = 0;
            $totalColumns = 0;
            $tmpOffsets = [];
            ...
            do {
                ...
                switch ($code) {
                    ...
                    case self::RECORD_MERGEDCELLS:
                        ...
                        ksort($mergeCells);
                        ksort($rowMergeCells);
                        $this->getSheetByIndex($index)->setMergeCells($mergeCells);
                        ...
                        if (isset($url)) {
                            for ($rowIndex = $firstRowIndex; $rowIndex <= $lastRowIndex; $rowIndex++) {
                                for ($columnIndex = $firstColumnIndex; $columnIndex <= $lastColumnIndex; $columnIndex++) {
                                    ...
                                    $this->getSheetByIndex($index)->getHyperlink($coordinate)->setUrl($url);
                                }
                            }
                        }
                        break;
                    case self::RECORD_QUICKTIP:
                        ...
                        for ($rowIndex = $firstRowIndex; $rowIndex <= $lastRowIndex; $rowIndex++) {
                            for ($columnIndex = $firstColumnIndex; $columnIndex <= $lastColumnIndex; $columnIndex++) {
                                $coordinate = Coordinate::columnLetterFromColumnIndex($columnIndex).($rowIndex + 1);
                                $this->getSheetByIndex($index)->getHyperlink($coordinate)->setTooltip($tooltip);
                            }
                        }
                        break;
                }
    ...
    protected function readRowOffsets(): void
    {
        ...
        foreach ($this->sheetsData as $index => $sheet) {
            ...
            do {
                ...
                switch ($code) {
                    case XlsReader:: RECORD_BLANK:
                    case XlsReader:: RECORD_MULBLANK:
                    case XlsReader:: RECORD_RK:
                    case XlsReader:: RECORD_MULRK:
                    case XlsReader:: RECORD_NUMBER:
                    case XlsReader:: RECORD_BOOLERR:
                    case XlsReader:: RECORD_LABEL:
                    case XlsReader:: RECORD_LABELSST:
                    case XlsReader:: RECORD_FORMULA:
                        $rowIndex = Helper::getInt($position, $this->workbook, 2);
