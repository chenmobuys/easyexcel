...
class XlsxReader extends Reader
{
    ...
    public static function readable(string $filename): bool
    {
        ...
        if ($zip->open($filename) === true) {
            try {
                $relsXml = static::getXMLReaderFromName(self::FILE_RELS_RELS, $filename);
                while ($relsXml->read()) {
                    if (
    ...
    protected function getRowIteratorBySheet(
        ...
        return new XlsxReaderRow($handler, $sheet, $startRow, $endRow);
    }
    ...
    protected function loadFromFile(string $filename): ReaderInterface
    {
        ...
        if (
            ...
            $workbookXml = static::getXMLReaderFromName($mainRelationships[self::NS_OFFICE_DOCUMENT], $this->filename);
            ...
            while ($workbookXml->read()) {
                ...
                $sheet = $this->getSheetByName($name, true);
                ...
                $sheetXml = static::getXMLReaderFromName($sheetXmlPathname, $this->filename);
                while ($sheetXml->read()) {
        ...
        if (isset($workbookRelationships[self::NS_SHARED_STRINGS])) {
            $sharedStringsXml = static::getXMLReaderFromName($workbookRelationships[self::NS_SHARED_STRINGS],
                $this->filename);
            $cacheValue = '';
                ...
                switch ($sharedStringsXml->name) {
                    case 'si':
                        if ($sharedStringsXml->nodeType == XMLReader::END_ELEMENT) {
                            $this->addSharedString($cacheValue);
                            $cacheValue = '';
                        }
                        break;
                    case 't':
    ...
    private function getMainRelationships(): array
    {
        if (empty($this->mainRelationships)) {
            $relsXml = static::getXMLReaderFromName(self::FILE_RELS_RELS, $this->filename);
    ...
    private function getWorkbookRelationships(): array
    {
        if (empty($this->workbookRelationships)) {
            ...
            $workbookRelsXml = static::getXMLReaderFromName($workbookRelsFilepath, $this->filename);
    ...
    private function getHyperlinkRelationshipsBySheetIndex(int $sheetIndex): array
    {
        ...
        if (!isset($this->hyperlinkRelationships[$sheetIndex])) {
            $sheetRelsXml = static::getXMLReaderFromName($sheetRelsXmlFilename, $this->filename);
