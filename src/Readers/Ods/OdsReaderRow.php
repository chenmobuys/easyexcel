...
class OdsReaderRow extends ReaderRow
{
    /**
     * XML path.
     *
    ...
    private function getNextRow(): array
    {
        ...
        do {
            ...
            if (!$this->xml->isEmptyElement) {
                ...
                while ($this->xml->read()) {
                    if ($this->xml->localName == OdsReader::EL_TABLE_CELL) {
                        break;
                    }
                }
            }
            $columnIndex += $numberColumnsRepeated;
        } while ($this->xml->next() && $this->xml->localName == OdsReader::EL_TABLE_CELL);
    ...
    public function rewind(): void
    {
        ...
        $sheetIndex = 0;
        while ($this->xml->read()) {
            if ($this->xml->localName != OdsReader::EL_TABLE) {
                continue;
            }
            do {
                if ($sheetIndex == $this->sheet->getIndex()) {
                    if (!$this->xml->isEmptyElement) {
                        while ($this->xml->read()) {
                            if ($this->xml->localName == OdsReader::EL_TABLE_ROW) {
                                break;
                            }
                        }
                    }
                ...
                $sheetIndex++;
            } while ($this->xml->next() && $this->xml->localName == OdsReader::EL_TABLE);
        }
        ...
        if ($this->startRow > 1) {
            ...
            } while ($this->xml->next() && $this->xml->localName == OdsReader::EL_TABLE_ROW);
        }
    }
